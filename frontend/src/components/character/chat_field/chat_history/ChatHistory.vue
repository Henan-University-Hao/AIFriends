...
const scrollRef = useTemplateRef('scroll-ref') // 处理自动滚动
...
async function loadMore() {  //流式加载聊天记录
  ...
  try {
    ...
    if (newMessages.length === 0) {
      ...
      for (const m of newMessages) { //注意for...of枚举元素（for....in枚举下标）
        //在上翻聊天记录时候，先看到的事ai的回复，然后是用户给ai发的消息
        // pushFrontMessage：处理消息前置插入
...
async function scrollToBottom() { // 滚动到底部
  await nextTick()
